...
value_slider_num.value = window.hty_fast_store.store_getData("view_Zoom.R").toFixed(2)
value_slider.value = parseFloat(value_slider_num.value)

window.hty_fast_store.store_onUpdate("view_Zoom", (event, new_value) => {
  value_slider.value = parseFloat(new_value.R);
  value_slider_num.value = new_value.R.toFixed(2);
});

function slider_change(val) {
  window.hty_fast_store.store_saveData("view_Zoom.R", val);
  window.hty_pro_control.ipcSend("view_setZoom", ["view_Zoom.R", val]);
  // console.log(val)
}

// 当 store key 改变时，进行方法
// window.hty_fast_store.store_onUpdate('iframeZoom', (event, new_value) => {
