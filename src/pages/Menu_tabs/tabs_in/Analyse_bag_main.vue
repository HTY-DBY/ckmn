...
const src_ALL = ref(window.hty_fast_store.store_getData('Analyse_src'))
const tab = ref(window.hty_fast_store.store_getData('view_R_now_ID'))
const icon_size = '34px'

watch(tab, (val) => {
  // console.log(val)
  window.hty_fast_store.store_saveData('view_R_now_ID', val)
})
...
window.hty_fast_store.store_onUpdate("Analyse_src", (event, new_value) => {
  src_ALL.value = new_value
});

function hty_click(item) {
  if (item?.URL ?? false) {
    window.hty_pro_control.ipcSend("view_loadURL", ["L_win", item?.URL]);
    // window.hty_fast_store.store_saveData("view_url.L", item.URL);
  } else {
    window.hty_pro_control.ipcSend("view_loadURL", ["L_win", 'https://www.bilibili.com/404']);
  }
